...
- `data/construction-sites.json` - строящиеся объекты (адреса, этапы, проценты)
...
Источник данных: `data/construction-sites.json`.

Обновлять можно без правок кода:

- `updatedAt` - дата обновления списка
- `activeCount` - сколько объектов сейчас строится в блоке контактов
- `sites[].name` - название объекта
- `sites[].address` - адрес объекта
- `sites[].stage` - текущий этап работ
- `sites[].progress` - готовность в процентах (0-100)

После сохранения JSON блок на главной странице обновится автоматически.
